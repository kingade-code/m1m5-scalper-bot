...
"""Singular trailing-config backtest under HARD fills friction.
Same as trail_sweep_worker but with spread 0.60 + stop-slippage 0.25
(the live-fill audit measured real SL fills landing ~0.48 points beyond
the modelled stop - i.e. reality is roughly DOUBLE the assumed 0.40).
Writes `_res_fric_<label>_<key>.json`.
"""
import json
import os
import sys
...
bt.USE_OPEN_RR = False  # friction sweep tests classic TP/ATR-trail exits
...
out_name = f"_res_fric_{label}_{key.replace('/', '_')}.json"
