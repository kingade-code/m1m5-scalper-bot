...
"""Trailing-grid re-run under HARD friction (0.60 spread / 0.25 stop-slip),
mirroring what the live-fill audit says real XAUUSD fills cost. Baseline
0.30/0.10 numbers already live in trail_comparison.json; this produces
friction_comparison.json so the ranking can be diffed at realistic costs.
Resumable via checkpointing, runs isolated subprocesses.
"""
import json
import subprocess
import sys
...
OUT = os.path.join(BASE, "friction_comparison.json")
...
def _merge(data, label, key):
    f = os.path.join(BASE, f"_res_fric_{label}_{key.replace('/', '_')}.json")
    ...
    for tf, months, mode, label in LEGS:
        ...
        for s, st, key in configs():
            if key in data[label]:
                ...
                continue
            cmd = [sys.executable, "friction_sweep_worker.py",
