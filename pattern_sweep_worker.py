...
"""Single (leg, pattern) backtest worker. Writes results to a per-pattern
JSON file that pattern_sweep.py merges into the checkpoint. Runs entirely
inside its own process/MT5 connection so failures are isolated.
"""
import json
import os
import sys
...
import pattern_detector as pdm  # noqa: E402
import chart_patterns as cp  # noqa: E402
import config as cfg  # noqa: E402
from pattern_sweep import SPREAD, SLIP, cached_get_ohlc  # noqa: E402
...
label = sys.argv[4]
pattern = sys.argv[5]
monkey = not pattern.startswith("current")
...
if monkey:
    df = bt.get_ohlc("XAUUSD", tf, 96 * 22 * months + 500)
    lut = cp.precompute(df, pattern)

    def detect(window, symbol, lut=lut):
        return lut.get(window.iloc[-1]["time"], 0)

    pdm.detect_pattern = detect
...
with open(os.path.join(BASE, f"_res_{label}_{pattern}.json"), "w") as f:
    json.dump(metrics, f)
print(f"{label}/{pattern}: {metrics['net_pct']}% "
